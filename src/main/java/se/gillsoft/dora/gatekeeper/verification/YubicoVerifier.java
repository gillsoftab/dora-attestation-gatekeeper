package se.gillsoft.dora.gatekeeper.verification;

import org.bouncycastle.asn1.*;
import org.springframework.stereotype.Component;
import se.gillsoft.dora.gatekeeper.model.HsmVendor;
...
public class YubicoVerifier implements HsmAttestationVerifier {
    ...
    private static final String YUBICO_ROOT_CA = """
            ...
            
            -----END CERTIFICATE-----""";
    ...
    public YubicoVerifier() {
        try {
            ...
            ByteArrayInputStream is = new ByteArrayInputStream(YUBICO_ROOT_CA.trim().getBytes());
            rootCa = (X509Certificate) cf.generateCertificate(is);
        } catch (Exception e) {
            // Root CA will be loaded from chain instead
            rootCa = null;
        }
    }
    ...
    public boolean verifyAttestation(X509Certificate attestationCert, PublicKey csrPublicKey) {
        try {
            ...
            return Arrays.equals(attestedKey.getEncoded(), csrPublicKey.getEncoded());
        } catch (Exception e) {
            return false;
        }
    }
    ...
    public YubicoAttestationResult verifyYubicoAttestation(
        ...
        try {
            ...
            if (Arrays.equals(attestedKey.getEncoded(), csrPublicKey.getEncoded())) {
    ...
    private boolean verifyCertChainToRoot(X509Certificate[] chain) {
        try {
            ...
            // Verify last cert against Yubico root
            X509Certificate last = chain[chain.length - 1];
            last.checkValidity();

            // Check if last is root or signed by root
            String issuer = last.getIssuerX500Principal().getName();
            if (issuer.contains("Yubico") && issuer.contains("Root")) {
                if (rootCa != null) {
            ...
            // Self-signed root in chain
            String subject = last.getSubjectX500Principal().getName();
            if (subject.contains("Yubico") && subject.contains("Root")) {
                return true;
            }
